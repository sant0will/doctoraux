    ...
    <div class="row">
		<div class="col-md-10 col-md-offset-1">
			<div class="box box-primary">
				...
				<form class="form-horizontal" method="post" action="{{action('OperadorController@update', $id)}}">
                    @csrf
					...
					<div class="box-body">
                            ...
                            <div class="col-sm-10">
                                <select id="cidadao" class="form-control" name="cidadao" required>
                                    @foreach($cidadaos as $cidadao)
                                        <option @if($operador->cidadao_id == $cidadao->id) selected @endif value="{{ $cidadao->id }}">{{ $cidadao->nome }}</option>
                                    @endforeach
                                </select>
                        ...
                        <div class="form-group has-feedback {{ $errors->has('codigo_operador') ? 'has-error' : '' }}">
							...
							<div class="col-sm-10">
                                <input type="emais" class="form-control" value="{{ $operador->codigo_operador }}" name="codigo_operador" id="codigo_operador" placeholder="" minlength="1" maxlength="45" required>
